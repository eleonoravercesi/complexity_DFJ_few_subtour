...
for minimalize in [True, False]:
    # If the file pickle is already created, don't run the algorithm again
    try:
        with open("OUT_TSPLIB_minimalize_{}.pickle".format(minimalize), "rb") as f:
            out = pickle.load(f)
        print("TSPLIB_minimalize_{}.pickle already exists".format(minimalize))
    except:
        out = {}
...
for minimalize in [True, False]:
    try:
        with open("OUT_HardTSPLIB_TSPLIB_minimalize_{}.pickle".format(minimalize), "rb") as f:
            out = pickle.load(f)
        print("HardTSPLIB_TSPLIB_minimalize_{}.pickle already exists".format(minimalize))
    except:
        out = {}
...
for minimalize in [True, False]:
    try:
        with open("OUT_HardTSPLIB_random_minimalize_{}.pickle".format(minimalize), "rb") as f:
            out = pickle.load(f)
        print("HardTSPLIB_random_minimalize_{}.pickle already exists".format(minimalize))
    except:
        out = {}
